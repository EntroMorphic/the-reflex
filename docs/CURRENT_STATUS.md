...
**Last Updated:** September 2, 2026 — Audit remediation pass (`docs/AUDIT_SEP2026.md`): 25 findings across claim integrity, mechanism correctness, verified strengths, and reproducibility. Claims corrected in README, all three papers, ROADMAP, and the apr11 data digest. Code fixes landed in `gie_engine.c` / `test_kinetic.c` (group→pattern mapping applied to gate bias and fire counters; TriX/CPU agreement metric fixed; `MASK_SEQUENCE_INPUT` ablation flag added; reserved-but-unwired symbols labelled). Raw experiment logs are now tracked in git. **Still open:** UART-only verification (blocker since March 19); the `trix-integration` branch (May 16) remains unmerged and undocumented below.

**Prior update:** April 12, 2026 — Three papers rewritten around MTFP metrics and honest negative results (kinetic attention harmful at -5.5/80, Hebbian noise at +0.1 ± 1.1). Test verdict logic replaced with MTFP-based gates (`f327470`). Label-free TEST 14C validated on silicon: ablation regression confirmed, 15/15 TriX@15 (`data/apr11_2026/t14c_labelfree_seed_a.log`). Repo cleaned: archive consolidated, delta-observer gitlink removed, deprecated apr8 logs untracked, git gc 84→4.7 MB (`f4dbec8`). Remaining blocker: UART-only verification.

---

## Executive Summary

The Reflex is a three-layer ternary neural computing system built from commodity peripheral hardware. It classifies wireless signals using peripheral-hardware ternary dot products — 31–32/32 (96–100%) label-free on windowed classification across 15 runs, 90.5–96.4% per admitted packet — at 430 Hz measured with the CfC blend active, drawing ~30 µA in autonomous mode (datasheet; direct measurement still pending). Beneath that classifier, a sub-conscious layer on the LP core accumulates a temporal model of what the system has been perceiving — pattern-specific internal representations that develop from VDB episodic memory retrieval over 90 seconds of live operation.

**The reframe (March 23, 2026):** The Reflex is not building a better classifier. It is building a temporal context layer beneath the classifier. Every downstream contribution — kinetic attention, multi-agent coordination, Hebbian learning — earns its meaning from the quality and independence of that temporal model.

**Current state: 14/16 PASS (label-free).** Label-free classification of 4 well-separated patterns (commit `c7ef286`): 31–32/32 windowed (96–100% across 15 runs), 90.5–96.4% per admitted packet, with essentially all error in P3. VDB episodic memory produces 8.5-9.7/80 MTFP divergence — causally necessary (TEST 13 ablation). TEST 14 FAIL: kinetic attention gives no improvement (-5.5 ± 5.3/80, n=3; harmful in 2 of 3 runs; not distinguishable from zero). TEST 15 FAIL: Hebbian +0.1 ± 1.1/80, n=3 (noise). Both are honest negative results reported in the papers. VDB stabilization confirmed label-free (TEST 14C: ablation regression, 15/15 TriX@15). Structural wall verified across all experiments.
...
- April 9-11: Full audit + remediation session. (1) Test harness split into per-area files (`5657e92`). (2) Constants deduplicated, 66 inactive C files archived, vestigial extern hoisted. (3) **Second compounding bug found:** sender enrollment starvation in transition mode — Board A only saw P1 during enrollment, making sig[0]/sig[2]/sig[3] zero. Fixed (`63877f7`). All prior multi-seed data invalidated; apr8 data deprecated (`data/apr8_2026/DEPRECATED.md`). (4) Multi-seed re-run with corrected sender: A=15/15, B=8/15, C=15/15 (`data/apr9_2026/SUMMARY.md`). (5) **Bias release mechanism** correctly traced as geometric ×0.9/step (not "within 4 steps"). "4" was a disagree-count trit threshold, not a step count. Hard disagree-zero path not exercised on clean seeds. (6) **Label-free accuracy** experiment (R3a): pattern_id trits [16..23] were undisclosed "primary discriminator" in signatures. With label masked: 71% accuracy (P2 10%, all others 100%). Root cause: P2 payload shared 48/64 trits with P1. Fix: distinct P2 payload (`c7ef286`). Result: **100% label-free (32/32 windowed) on this run, 14/14 PASS**. RSSI noise hypothesis disproved (masking RSSI gave 68%). *(Sep 2026 audit: across all 15 post-fix label-free runs the windowed figure is 31-32/32 = 96-100%, and per-admitted-packet accuracy is 90.5-96.4%.)* (7) Self-adversarial red-team produced 12 items (`docs/REMEDIATION_PLAN_APR09_REDTEAM.md`, local-only). (8) **Pillar 3 implemented:** LP Hebbian weight learning (`lp_hebbian_step()`, commit `32fb061`). Ablation-controlled TEST 15 confirmed +2.5 Hamming over control (`4343447`). **BUT H2 found label leakage** (`a0d3a36`): the +2.5 was from pattern_id leaked through GIE hidden state. With genuinely label-free input (`MASK_PATTERN_ID_INPUT=1`), Hebbian contribution is -1.7 (harmful). VDB mismatch error signal is label-dependent. (9) **Key secondary finding:** removing pattern_id from GIE input improved VDB-only LP divergence from 0.7 to 3.3/16. The label was drowning out discriminative features. Recommended operating mode: `MASK_PATTERN_ID_INPUT=1`. (10) **TriX-output Hebbian (v2):** Replaced VDB mismatch with TriX-labeled accumulator (population mean). Still f-only. Result: -1.0 label-free (less harmful but still negative — f-pathway-only flips adjust the gate but not the candidate). (11) **Diagnosed Hebbian (v3, commit `427fea3`):** Three LMM cycles identified the missing atomic — DIAGNOSIS. Per neuron: is the error in f (gate) or g (candidate)? Compare |f_dot| vs |g_dot|, fix the cheaper pathway. Single-run: +1.3 Hamming (Control=0.7, Hebbian=2.0). But 3-rep replication (`698231c`): sign +0.1 ± 1.2, MTFP +0.1 ± 1.1 — **indistinguishable from zero.** The diagnosis fixed the direction (v1 was -1.7, v2 was -1.0, v3 is neutral) but the mechanism doesn't improve on VDB-only at either resolution. Structural wall intact. (12) **MTFP resolution measurement** (`ebc65a4`): MTFP baseline is 9.7 ± 0.6 /80 from VDB alone. Note on framing (Sep 2026 audit): 9.7/80 vs 0.7/16 is not a like-for-like ratio. Normalized, that is 12.1% of trits vs 4.4% — about 2.8×, not the 12× the raw counts suggest. The defensible statement is that MTFP has 5× the dimensions and resolves effects sign-space hides. (13) **Kinetic attention does not improve on the baseline at MTFP** (`5959466`): Test 14 MTFP data shows per-run improvements of +0.4, -7.0, -9.8 — harmful in 2 of 3 runs, mean -5.5 ± 5.3/80, t(2) = -1.80, p ~ 0.21, i.e. not distinguishable from zero at n=3. Group 3 never fires the gate in any condition of any run, so the bias reached 3 of 4 pattern groups. The sign-space "improvement" (+1.3/16) was an artifact of trading magnitude diversity for sign diversity. The bias saturates GIE hidden state, reducing LP dot magnitude diversity. Remaining: paper rewrites with honest MTFP numbers, multi-seed re-run, UART verification.
...
|-------|----------|------|-------|-------------|
| GIE | GDMA+PARLIO+PCNT peripherals | 430 Hz (blend active) | ~0 CPU | 64-neuron CfC inference via peripheral routing |
...
| VDB M4: NSW Graph | 6/6 | M=7, ef=32, dual entry, recall@1 19/20=95%, recall@4 72/80=90% (fixed query seed) | `7db919f` |
...
| GIE free-running | Verified | 430 Hz blend-active (classification-mode rate unmeasured), 64 neurons, ~0 CPU |
| LP core CfC | Verified | 100 Hz, 16 neurons, ~30uA |
| LP core VDB | Verified | 64 nodes, NSW graph, recall@1 19/20=95% (n=20, fixed seed, at threshold) |
...
│                              │                                   │
│   430 Hz (CfC blend active), CPU-free after init                 │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
