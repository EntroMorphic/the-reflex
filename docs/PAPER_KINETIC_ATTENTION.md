...
**CPU core_pred (HP core, ~4 Hz).** The HP core computes `argmax(dot(input, sig[p]))` using the same signatures as a scalar dot product. This classifier is used in TEST 12-14 for LP feedback dispatch and novelty gating. Its accuracy is approximately 80% — the P0-P1 signature overlap (73% cross-dot ratio) leaves a discrimination margin of ~28 points, which per-packet variation in timing and RSSI trits can exceed.

**Why not TriX dispatch?** The TriX ISR computes per-group dot products from the GDMA circular chain. A GDMA chain offset means group index g does not necessarily correspond to pattern g — the ISR cannot determine which group is which pattern without CPU-side resolution. Direct TriX dispatch was tested (April 8, 2026) and produced correct classification under the 2-pattern transition sender (where the offset doesn't matter for argmax), but misclassified all inputs as P3 under the 4-pattern normal sender. The GDMA offset resolution requires matching ISR group scores against CPU-computed pattern dots, which the trix_channel path already implements for diagnostic verification. For LP feedback dispatch, CPU core_pred is used because it computes dots directly from sig[] without GDMA offset ambiguity.
...
**Accumulator contamination (known limitation).** The 20% misclassification rate is concentrated on the P0-P1 pair (73% cross-dot ratio), producing systematic cross-contamination in the P0 and P1 LP accumulators. The sign-of-sum convergence argument holds for trits where the correct pattern dominates, but near-threshold trits (typically 2-4 out of 16) may be affected. Direct TriX dispatch was investigated (April 8, 2026) to eliminate this contamination, but was not viable due to the GDMA chain offset — the ISR cannot map group indices to pattern IDs without CPU-side resolution. The contamination is mitigated by the ternary disagree-count agreement, which detects prior-evidence conflict at the trit level rather than relying on the accumulator's absolute accuracy. Future work: resolve the GDMA offset mapping at enrollment time and store it for ISR use, enabling TriX dispatch without CPU-side pattern identification.
...
7. **LP feedback uses CPU core_pred (~80% accuracy).** Direct TriX dispatch was investigated to extend the structural classification guarantee to the LP accumulation pathway, but is not viable due to the GDMA chain offset — the ISR group index does not map to pattern ID without CPU-side resolution. The P0-P1 cross-contamination (73% cross-dot ratio) is mitigated by the ternary disagree-count agreement, which detects conflict at the trit level. Resolving the GDMA offset at enrollment time would enable TriX dispatch in future firmware.
