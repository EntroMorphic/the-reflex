...
\textbf{Layer 1 (GIE):} A Geometry Intersection Engine running ternary dot products on peripheral hardware (GDMA $\to$ PARLIO $\to$ PCNT loopback) at 430\,Hz with the CfC blend active. Classification runs with the blend disabled, where the ISR skips the hidden-state re-encode and the loop rate is higher but unmeasured (see Stratum~1, Limitations). Classifies wireless signals at 96--100\% label-free on windowed classification and 90.5--96.4\% per admitted packet (TriX ISR, structural guarantee: $W_f[\text{hidden}] = 0$).
...
Two cautions on reading these numbers. First, 9.7/80 and 0.7/16 are Hamming counts over spaces of different dimensionality; normalized, they are 12.1\% and 4.4\% of available trits, a ratio of $\sim$2.8$\times$, not the $\sim$12$\times$ suggested by comparing the raw counts. Second, the 80-trit code is not of uniform capacity: its fifth trit is degenerate for small dot magnitudes, which is where most LP dots fall (see Stratum~1, Limitations). MTFP is the right instrument for cross-condition comparison; it is not a calibrated distance.
...
\section{Kinetic Attention: No Improvement on the Baseline}

Agreement-weighted gate bias was tested under label-free conditions. Three runs of Test~14 gave MTFP improvements of $+0.4$, $-7.0$ and $-9.8$/80: harmful in 2 of 3 runs, mean $-5.5 \pm 5.3$, $t(2) = -1.80$, $p \approx 0.21$. At $n=3$ this is not distinguishable from zero, and we hold it to the same standard as the Hebbian result above. What the data supports is that gate bias does not improve on the VDB baseline.

Where it does act, the mechanism plausibly saturates the GIE hidden state, reducing LP dot magnitude diversity---the sign-space metric ($+1.3$/16) showed improvement by trading magnitude diversity for sign diversity. Two scope limits apply: the bias never reaches neuron group~3, which does not fire the gate in any condition of any run, and the three conditions always run in the same order, so drift is confounded with condition. Reported as an honest negative result with its scope stated.
...
\item \textbf{MTFP metric is nonlinear,} and the ``/80'' denominator overstates its effective resolution (see Stratum~1, Limitations).
\item \textbf{Both negative results are underpowered at $n=3$.} Neither kinetic attention nor Hebbian learning separates from zero. They support ``no improvement on the VDB baseline'' and nothing stronger.
\item \textbf{Test~14 condition order is not counterbalanced,} so drift across the 6-minute sequence is confounded with condition.
\item \textbf{Kinetic attention was never tested on a full network.} Neuron group~3 never fires the gate, so the bias modulates 3 of 4 pattern groups.
\end{enumerate}
