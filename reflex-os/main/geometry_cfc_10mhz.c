/*
 * geometry_cfc_10mhz.c — Ternary CfC at 10 MHz PARLIO clock
 *
 * Milestone 9: Performance verification at 10× clock rate.
 *
 * Copy of geometry_cfc.c (M7) with PARLIO at 10 MHz instead of 1 MHz.
 * Tests whether PCNT can keep up with the faster edge stream.
 *
...
static void init_parlio(void) {
    parlio_tx_unit_config_t cfg = {
        ...
        .clk_in_gpio_num = -1,
        .output_clk_freq_hz = 10000000,  /* 10 MHz — M9 test */
        .data_width = 2,
        .clk_out_gpio_num = -1,
        .valid_gpio_num = -1,
        .trans_queue_depth = 4,
...
static dot_result_t gie_dot_split(const int8_t *weights,
    ...
    /* At 10 MHz: 64 bytes = 256 symbols @ 0.1us = 25.6us + margin */
    int wait_us = (total_bytes * 4 * 2) / 10 + 100;
...
void app_main(void) {
    ...
    printf("\n");
    printf("============================================================\n");
    printf("  TERNARY CfC @ 10 MHz — Performance Verification\n");
    printf("  Milestone 9: 10× clock rate (1 MHz → 10 MHz)\n");
    ...
    etm_force_clk();
    printf("[INIT] PARLIO TX (2-bit, 10MHz, loopback)...\n");
    init_parlio();
